...
app = FastAPI(
    title="REST API",
    description="Реализация конвейера курсовой работы (ПЗ 1-8) — Глубокий анализ"
)
...
def run_heavy_pipeline(job_id: str, video_path: str):
    ...
    frame_idx = 0
    sample_rate = int(fps)  # Сканируем каждую секунду видео
        ...
        for segment in audio_results.get("segments", []):
            ...
            if any(word in text for word in drugs_keywords):
                raw_detections.append({
                    ...
                    "class": "DRUGS", "subclass": "drugs", "confidence": 0.92, "type": "audio"
                })

            if any(word in text for word in smoking_keywords):
                raw_detections.append({
                    "startFrame": start_frame, "endFrame": end_frame,
                    "class": "DRUGS", "subclass": "smoking", "confidence": 0.89, "type": "audio"
                })

            if any(word in text for word in alcohol_keywords):
                raw_detections.append({
                    "startFrame": start_frame, "endFrame": end_frame,
                    "class": "DRUGS", "subclass": "alcohol", "confidence": 0.91, "type": "audio"
                })
    ...
    def merge_intervals(detections):
        ...
        for current in detections[1:]:
            ...
            if (current['type'] == prev['type'] and
                    ...
                    current['startFrame'] <= prev['endFrame'] + (fps * 3)):  # Склеиваем, если разрыв < 3 секунд
...
def get_openapi_yaml():
    ...
    if not os.path.exists(yaml_path):
        raise HTTPException(status_code=500, detail=f"Критическая ошибка: Файл спецификации '{yaml_path}' отсутствует!")
